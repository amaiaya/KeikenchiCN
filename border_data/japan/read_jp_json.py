...
def export_boundaries_csv(items, outpath):
    ...
    for props, geom in items:
        path_list = ['日本', props['KEN'], props['SICHO'], props['GUN'], props['SEIREI'], props['SIKUCHOSON']]
...
if __name__ == '__main__':
    ...
    export_boundaries_csv(items, 'japan_boundaries_wgs.csv')
    convert_to_gcj_and_export(items, 'japan_boundaries_gcj.csv')

    # 列出可选项，让用户挑选一个进行可视化
    for i, (props, geom) in enumerate(items):
        name = props['KEN']+props['SIKUCHOSON']
        print(f"[{i}] {name}")
