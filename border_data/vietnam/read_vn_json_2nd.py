...
def export_boundaries_csv(items, outpath):
    ...
    for props, geom in items:
        path_list = ['Vietnam', props['ten_tinh'].replace(' ','_'), props['ten_xa'].replace(' ','_')]
...
if __name__ == '__main__':
    ...
    export_boundaries_csv(items, 'vn_2_boundaries_wgs.csv')
    convert_to_gcj_and_export(items, 'vn_2_gcj.csv')

    # 列出可选项，让用户挑选一个进行可视化
    for i, (props, geom) in enumerate(items):
        name = props['ten_tinh']+', '+props['ten_xa']
        print(f"[{i}] {name}")
