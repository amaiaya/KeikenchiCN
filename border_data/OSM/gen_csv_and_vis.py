...
def export_boundaries_csv(items, outpath):
    ...
    """
    country_name = outpath.replace('_boundaries_wgs.csv','')
    rows = []
    for props, geom in items:
        # print(props)
        admin1 = props['admin1_name'].replace(' ','-') if props['admin1_name'] else 'null'
        admin2 = props['admin2_name'].replace(' ','-') if props['admin2_name'] else 'null'
        path_list = [country_name, admin1, admin2]
...
if __name__ == '__main__':
    ...
    export_boundaries_csv(items, f'{path.replace("_admin2.geojson","")}_boundaries_wgs.csv')
    # convert_to_gcj_and_export(items, 'nk_boundaries_gcj.csv')

    # 列出可选项，让用户挑选一个进行可视化
    for i, (props, geom) in enumerate(items):
        name = str(props['admin1_name'])+', '+str(props['admin2_name'])
        print(f"[{i}] {name}")
